...
export function Notecard({ note, onNoteDelete}: NoteCardProps) {
    ...
    <Dialog.Root>
        <Dialog.Trigger className='rounded-md text-left flex-col bg-slate-800 p-5 gap-3 overflow-hidden relative outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400 '>
            ...
            <p className='text-sm leading-6 text-slate-400'> {note.content}
            </p>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className='fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none'>
                    ...
                    <div className='felx flex-1 flex-col gap-3 p-5 '>
                        <span className='text-sm font-medium text-slate-300'> {formatDistanceToNow(note.date, {locale: ptBR, addSuffix: true})} </span>
                    ...
                    {/* coloquei group em dois lugares para formar um grupo e dentro desse grupo posso modificar para que algo aconteça usando ambos, exp ficar com underline ao passar no span apagar a nota e com grupo ficar com underline no span mais ao passar mouse no botão */}
                </Dialog.Content>
            </Dialog.Portal>
    </Dialog.Root>
    )
